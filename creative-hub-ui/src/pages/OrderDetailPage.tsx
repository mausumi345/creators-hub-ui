...
const OrderDetailPage = () => {
  ...
  const paymentModeBadge = (() => {
    if (!order.escrow_id) {
      return {
        label: "No escrow yet",
        className: "bg-white/10 text-white/60 border border-white/15",
      };
    }
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-gray-950 via-gray-900 to-gray-950 pt-20 px-4 pb-10">
      ...
      {isCancelled ? (
        <div className="bg-white/5 backdrop-blur-sm border border-red-500/30 rounded-2xl p-6 mb-6">
          <div className="text-red-300 font-semibold mb-2">Order cancelled</div>
          <div className="text-white/70 text-sm">
            {cancelEvent?.details?.reason
              ? `Reason: ${cancelEvent.details.reason}`
              : "Reason not provided."}
          </div>
          {cancelEvent?.details?.cancelled_by && (
            <div className="text-white/50 text-xs mt-2">
              Cancelled by: {cancelEvent.details.cancelled_by}
            </div>
          )}
        </div>
      ) : (
      ...
      <div className="bg-white/5 backdrop-blur-sm border border-white/10 rounded-2xl p-6 mb-6">
        <div className="flex items-center justify-between mb-4">
          <div className="text-white font-semibold">Materials Shipment</div>
          {shipment ? (
            <span className="text-xs text-white/60">Status: {shipment.status}</span>
          ) : (
            <span className="text-xs text-white/60">Not shipped yet</span>
          )}
        </div>
        ...
        {shipment ? (
          <div className="space-y-3 text-sm text-white/80">
            <div className="grid sm:grid-cols-3 gap-3">
              <div className="p-3 rounded-lg bg-white/5 border border-white/10">
                <div className="text-white/50 text-xs">Carrier</div>
                <div className="text-white font-semibold">{shipment.carrier}</div>
              </div>
              <div className="p-3 rounded-lg bg-white/5 border border-white/10">
                <div className="text-white/50 text-xs">Receipt / Tracking #</div>
                <div className="text-white font-semibold">{shipment.tracking_number}</div>
              </div>
              <div className="p-3 rounded-lg bg-white/5 border border-white/10">
                <div className="text-white/50 text-xs">Shipped</div>
                <div className="text-white font-semibold">{formatDate(shipment.shipped_at || "")}</div>
              </div>
            </div>
            {shipment.notes && (
              <div className="text-white/60">Notes: {shipment.notes}</div>
            )}
            {shipment.status === "received" && (
          ...
          <div className="text-white/50 text-sm">No shipment recorded yet.</div>
        )}
          ...
          <div className="mt-4 grid grid-cols-1 md:grid-cols-3 gap-3">
            <div>
              ...
              <input
                ...
                className="mt-1 w-full rounded-lg bg-slate-900 border border-white/10 text-white p-2 text-sm"
                placeholder="DHL, DTDC, FedEx..."
              />
            </div>
            <div>
              <label className="text-xs text-white/60">Receipt / Tracking #</label>
              <input
            ...
            <div className="md:col-span-3 flex justify-end">
              <button
                ...
                className="px-4 py-2 rounded-lg bg-fuchsia-600 text-white text-sm hover:bg-fuchsia-500 disabled:opacity-50"
              >
        ...
        {isSeller && shipment && (
          ...
          {connectLink && (
            <div className="mt-3 p-3 rounded-xl border border-amber-400/40 bg-amber-500/10 text-amber-100 text-sm flex items-center justify-between gap-3">
              <div>
                <div className="font-semibold">Seller payouts not set up</div>
                <div className="text-amber-200/80 text-xs mt-1">Seller must complete Stripe Express onboarding to receive funds.</div>
              </div>
              <a
                className="px-4 py-2 rounded-lg bg-amber-500 text-black font-medium hover:bg-amber-400 transition"
